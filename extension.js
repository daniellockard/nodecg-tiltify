...
module.exports = function(nodecg) {
  ...
  async function askTiltifyForDonations() {
    var donationsRequest = await WebRequest.get(`https://tiltify.com/api/v3/campaigns/${nodecg.bundleConfig.tiltify_campaign_id}/donations`, {
      headers: {
  ...
  async function askTiltifyForTotal() {
    var donationTotalRequest = await WebRequest.get(`https://tiltify.com/api/v3/campaigns/${nodecg.bundleConfig.tiltify_campaign_id}`, {
      headers: {
        "Authorization": "Bearer " + nodecg.bundleConfig.tiltify_api_key
      }
    });
